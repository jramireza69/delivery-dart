...
class UsersProviders extends GetConnect {
  ...
  Future<ResponseApi> update(User user) async {
         ...
         if(response.body == null ) {
           Get.snackbar('Error', 'no se pundo actualizar ACTUALIZAR SIN IMAGEN');
           return ResponseApi();
         }
         //VALIDA QUE SI TENGA EL TOKEN
         if(response.statusCode == 500 ) {
           Get.snackbar('Error', 'no esta autorizado para editar valide con soporte');
           return ResponseApi();
         }
  ...
  * GET X
  ...
  Future<ResponseApi> login(String email, String password) async {
         ...
         );  //ESPERAR HASTA QUE EL SERVIDOR RETORNE LA DATA
         if(response.body == null ) {
